...
使用本 SDK 时，机器人版本必须 **大于等于 1.00.000**。低于该版本的机器人请先升级后再使用。
...
| 普通 ARM64 Linux 外部主机 | `aarch64_host` | 远端 High-level、音频采集和播放 | 原生构建或 GNU ARM64 交叉编译，指定 `-DPLATFORM=aarch64_host` |
...
Low-level 和本地视频/布局访问要求在大脑板运行；外部主机也支持远端 High-level 和音频。
...
在 x86_64 Linux 主机原生构建，运行库为 `lib/x86_64/`：
...
Python 安装步骤见 [Python SDK](https://github.com/uniubi-ai/uniubi_robot_sdk_py/blob/main/README.zh-CN.md).
