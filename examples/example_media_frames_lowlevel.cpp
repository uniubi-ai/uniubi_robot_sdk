/**
 * ========================================================
 *  @file example_media_frames_lowlevel.cpp
 *  @brief Motion SDK 媒体帧订阅示例：AudioFrame / VideoFrame / EncodedVideoFrame / 摄像头 IMU
 *  @note Low-level connect may switch ownership; exit restores factory mode. Do not run alongside another controller.
...
struct Options {
    ...
    std::string clientId = "mediaFrameLowLevelExample";
    std::string deviceId;
...
void printUsage(const char* program) {
    ...
    std::printf("example: %s - mediaFrameLowLevelExample - 0 0 10 eth0\n", program);
}
...
int main(int argc, char** argv) {
    ...
    if (!client) {
        std::fprintf(stderr, "create low level client failed\n");
        service->shutdown();
        return 1;
    }

    // Low-level connect may change ownership even without enabling motors.
    const auto cleanup = [&]() {
        const bool restored = client->restoreMotionControlMode();
        if (!restored) std::fprintf(stderr, "ERROR: restoreMotionControlMode failed: %d\n", int(client->getLastError()));
        client->disconnect();
        service->shutdown();
        return restored;
    };
    ...
    /// 音视频通道统一走 IMediaBusClient；本示例不发送电机控制帧。
